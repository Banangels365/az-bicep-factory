...
@description('Dev subscription IDs to associate')
param devSubscriptionIds array = []

@description('Staging subscription IDs to associate')
param stagingSubscriptionIds array = []
...
// Dev Landing Zone Management Group
module devMg 'management-group/main.bicep' = {
  ...
  params: {
    ...
    parentManagementGroupId: '${managementGroupPrefix}-landing-zones'
    subscriptionIds: devSubscriptionIds
...
// Staging Landing Zone Management Group
module stagingMg 'management-group/main.bicep' = {
  name: 'Contoso-staging-mg'
  scope: tenant()
  params: {
    managementGroupId: '${managementGroupPrefix}-staging'
    displayName: 'Contoso Staging'
    parentManagementGroupId: '${managementGroupPrefix}-landing-zones'
    subscriptionIds: stagingSubscriptionIds
    // description: 'Management group for staging workloads'
  }
  dependsOn: [
    landingZonesMg
  ]
...
// Production Landing Zone Management Group
...
module sandboxMg 'management-group/main.bicep' = {
  name: 'Contoso-sandbox-mg'
  scope: tenant()
  params: {
    managementGroupId: '${managementGroupPrefix}-sandbox'
    displayName: 'Sandbox'
    parentManagementGroupId: '${managementGroupPrefix}-root'
...
module decomMg 'management-group/main.bicep' = {
  name: 'Contoso-decom-mg'
  scope: tenant()
  params: {
    managementGroupId: '${managementGroupPrefix}-decommissioned'
    displayName: 'Decommissioned'
    parentManagementGroupId: '${managementGroupPrefix}-root'
    // description: 'Management group for decommissioned resources'
  }
  dependsOn: [
    rootMg
  ]
...
output managementGroupIds object = {
  ...
  platform: platformMg.outputs.managementGroupId
  landingZones: landingZonesMg.outputs.managementGroupId
  dev: devMg.outputs.managementGroupId
  staging: stagingMg.outputs.managementGroupId
  prod: prodMg.outputs.managementGroupId
  sandbox: sandboxMg.outputs.managementGroupId
  decommissioned: decomMg.outputs.managementGroupId
}
